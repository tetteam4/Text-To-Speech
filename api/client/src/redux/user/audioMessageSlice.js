...
export const fetchAudioMessages = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        error.message || "Error fetching audio messages"
      );
    }
  }
);
   ...
   export const createAudioMessage = createAsyncThunk(
    ...
    async (messageData , {rejectWithValue}) => {
        try {
          const response = await axios.post('/api/audioMessage/create', messageData, {
            withCredentials: true,
             headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
             },
  ...
  export const markMessageAsRead = createAsyncThunk(
    ...
    async (messageId, { rejectWithValue }) => {
          try {
            const response = await axios.put(`/api/audioMessage/mark-read/${messageId}`,{},{
              withCredentials:true,
                 headers:{
                     Authorization: `Bearer ${localStorage.getItem('token')}`
                 }
...
export const { clearMessages } = audioMessageSlice.actions

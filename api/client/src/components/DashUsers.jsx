...
export default function DashUsers() {
  ...
  const handleDeleteUser = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setUsers((perv) => perv.filter((user) => user._id !== userIdToDelete));
        setShowModal(false);
      }
    } catch (error) {
      console.log(error);
  ...
  return (
    <div className=" flex-1 table-auto overflow-x-scroll md:mx-auto p-3 scrollbarscrollbar-track-slate-100 scrollbar-thumb-slate-300 dark:scrollbar-track-slate-700 dark:scrollbar-thumb-slate-500 pt-16">
      {currentUser.isAdmin && users.length > 0 ? (
                  ...
                  <Table.Cell>
                    {user.isAdmin ? (
                      ...
                      <FaTimes
                        ...
                        onClick={() => {
                          // Toggle the isAdmin property of the user object
                          user.isAdmin = !user.isAdmin;
                          // Update the state to reflect the change
                          setUsers((prevUsers) =>
                            prevUsers.map((prevUser) =>
        ...
        <p className="text-center font-serif font-semibold"> No yet Users </p>
      )}
      <Modal
        show={showModal}
